...
裏面読み取りを実装する。表面解析後に裏面の有無を選択し、裏面画像は固定テンプレート補正を行わず自由記述OCRへ渡す。結果は確認画面のメッセージへ `【裏面】` 区切りで追加する。OCR未設定時は空欄・要確認として人間が編集できる。
...
実アンケート20〜30枚程度を使って精度評価・調整する。10枚程度の小規模評価は暫定検証とし、数字だけを目的に閾値を変更しない。
