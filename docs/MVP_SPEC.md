...
スマートフォンで書類のようにスキャンし、回答内容を読み取って確認・修正し、
既存Googleフォームへ人間が手動転記するコピー支援Webアプリを開発する。
...
- 現在の正式運用ではGoogleフォームへ人間が手動転記する。
- Google Sheets直接登録は通常運用では使用しない（既存providerは将来用に保持可能）。
...
確認画面から `Googleフォームを開く` で既存フォームを別タブに開ける。
フォームの編集権限は不要で、entry ID利用・自動入力・自動送信は行わない。

転記後は `次のアンケートを読み取る` で認識結果、編集内容、画像プレビュー、コピー状態をクリアする。
...
# 25. Google Sheets連携（現在の通常運用では使用しない）

Google Sheets直接登録は現在の正式運用ではない。既存providerとテストは将来の開発用として保持できるが、通常の確認画面から送信しない。

複数人がアプリを使用しても、通常はGoogleフォームへ各自が手動転記する。
Sheets連携を再利用する場合も、正式運用への切替を別途明示する。
...
## POST /api/submit（Sheets providerの参考API。通常フローでは使用しない）

確認済みのSurveyResultを受け取る。

Google Sheetsへ1行appendする。現在の通常フローからは呼び出さない。
...
# 37. Phase 7（参考実装・通常運用では使用しない）

Google Sheets providerは将来の選択肢として保持するが、現在の正式運用では使用しない。
確認済みデータはコピー支援画面から既存Googleフォームへ人間が転記し、Googleフォーム上で送信する。
